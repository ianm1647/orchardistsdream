...
public class ODCuttingBoardRecipeBuilder {
    private final List<ChanceResult> results = new ArrayList(4);
    private final Ingredient ingredient;
    private final Ingredient tool;
    private String soundEventID;

    private ODCuttingBoardRecipeBuilder(Ingredient ingredient, Ingredient tool, ItemLike mainResult, int count, float chance) {
        this.results.add(new ChanceResult(new ItemStack(mainResult.asItem(), count), chance));
        this.ingredient = ingredient;
        this.tool = tool;
    }

    public static ODCuttingBoardRecipeBuilder cuttingRecipe(Ingredient ingredient, Ingredient tool, ItemLike mainResult, int count) {
        return new ODCuttingBoardRecipeBuilder(ingredient, tool, mainResult, count, 1.0F);
    }

    public static ODCuttingBoardRecipeBuilder cuttingRecipe(Ingredient ingredient, Ingredient tool, ItemLike mainResult, int count, int chance) {
        return new ODCuttingBoardRecipeBuilder(ingredient, tool, mainResult, count, (float)chance);
    }

    public static ODCuttingBoardRecipeBuilder cuttingRecipe(Ingredient ingredient, Ingredient tool, ItemLike mainResult) {
        return new ODCuttingBoardRecipeBuilder(ingredient, tool, mainResult, 1, 1.0F);
    }

    public ODCuttingBoardRecipeBuilder addResult(ItemLike result) {
        return this.addResult(result, 1);
    }

    public ODCuttingBoardRecipeBuilder addResult(ItemLike result, int count) {
        this.results.add(new ChanceResult(new ItemStack(result.asItem(), count), 1.0F));
        return this;
    }

    public ODCuttingBoardRecipeBuilder addResultWithChance(ItemLike result, float chance) {
        return this.addResultWithChance(result, chance, 1);
    }

    public ODCuttingBoardRecipeBuilder addResultWithChance(ItemLike result, float chance, int count) {
        this.results.add(new ChanceResult(new ItemStack(result.asItem(), count), chance));
        return this;
    }

    public ODCuttingBoardRecipeBuilder addSound(String soundEventID) {
    ...
    public void build(Consumer<FinishedRecipe> consumerIn) {
        ...
        this.build(consumerIn, "orchardistsdream:cutting/" + location.getPath());
    }
    ...
    public void build(Consumer<FinishedRecipe> consumerIn, ResourceLocation id) {
        consumerIn.accept(new vectorwing.farmersdelight.data.builder.CuttingBoardRecipeBuilder.Result(id, this.ingredient, this.tool, this.results, this.soundEventID == null ? "" : this.soundEventID));
    }
