...
public class ChilledCookingPotScreen extends AbstractContainerScreen<ChilledCookingPotMenu> implements RecipeUpdateListener {
    ...
    protected void renderMealDisplayTooltip(GuiGraphics gui, int mouseX, int mouseY) {
        if (this.minecraft != null && this.minecraft.player != null && (this.menu).getCarried().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.hasItem()) {
            if (this.hoveredSlot.index == 6) {
                ...
                tooltip.add(Component.translatable("container.chilled_cooking_pot.served_on", container).withStyle(ChatFormatting.GRAY));
